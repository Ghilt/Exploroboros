...
describe('parse — precedence & structure', () => {
  ...
  it('parses a BARE registry read A (no brackets needed)', () => {
    ...
    // a bare registry with a .-path reads a neighbour's registry
...
describe('parse — attribute .-paths', () => {
  ...
  it('parses turns, straight and nearest-unvisited segments', () => {
    ...
    const b = ok('visited.back > 0')
    if (b.kind !== 'compare' || b.left.kind !== 'attr') throw new Error('expected an attr comparison')
    expect(b.left.path).toEqual([{ kind: 'back' }])
