...
// Snap every vertex to a shared canonical point so coincident endpoints become identical numbers
// (proximity-based, robust to grid straddle), letting stitch's endpoint keys pair shared edges.
function weldVertices(tiles: ReadonlyArray<{ vertices: Vec2[] }>, eps: number): void {
  const canon: Vec2[] = []
