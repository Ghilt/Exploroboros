...
//   only move if visited == 0                 → directive if visited@target > 0 always forbid move
//   only move if adjacent-visited-unique == 1 → directive if visited-neighbors@target != 1 always forbid move
//   move rel 0 / 1 / 7 / 2 / 6                → move straight / r1 / l1 / r2 / l2
// A prototype `only move if PRED` filters every following move by its TARGET tile. In the app a GATE is a
// `forbid` of the negation (`directive if <not PRED>@target always forbid move`) — an `allow` only OVERRIDES
// a forbid, so it can't gate on its own. The `@target` path reads the destination; without it the guard
// would read the current tile. The five moves are tried in that
