...
| 2026-06-28 | classic / ringlare / wedge-seek ported to the gallery via `orientation` (3 of the 4 rotation-routed fractals; sierpinski still deferred — its absolute-compass wedge routing doesn't map onto our ~22.5°-rotated frame) | ✅ yes | owner reviewed the gallery regenerations on 5238 and approved; each grows to a natural stop in the headless grow-check (classic 21%, ringlare 11% single-walker rings, wedge-seek 53%); re-tuned after the master session's wedge through-pairing change (classic gained a `move straight` fallback + a re-probed seed heading, ringlare a new seed heading) | `f6e64a8` |
