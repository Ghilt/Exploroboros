...
- **Record what's verified.** When the owner confirms a feature, add a row to the §7 verification log
  noting the feature + how it was verified. **Docs are part of the change: update §6/§7/§8/§9 (and any
  guide text) IN THE SAME commit as the code — never a separate "docs:" commit** (owner's call — the split
  is noise). Because the doc update rides in the commit, don't try to record that commit's own hash in the
  §7 row (it can't exist yet); leave the commit column `—` (git log + the date/feature identify it).
...
- **Commits:** only on the owner's say-so; to `main` unless told otherwise; the owner pushes. **No
  `Co-Authored-By` / co-authorship trailer in commit messages** (owner's call — it's noise). Keep messages
  to a clear subject + body.
...
- **Traverser DSL — `.`-chained moves, bare registries, `if {}` blocks, `find-tile` search, `exists.path`
  ...
  **base** (`f0`, `f1.e0`, `tile-type.f1`) but never a later hop (`move e0.f1` is rejected). **`exists.path`**
  ...
  legitimate zero/false) tests whether a `.`-path resolved to a real tile at all; works on any path, not just
...
- **Board numbering schemes + `find-lowest/highest-tile` (2026-07-08):** a **tile numbering** the user picks
  from a dropdown in a new **"canvas settings"** section at the foot of the Inspect pane (which also now hosts
  the **display-mode** control, moved out of the toolbar). Three schemes, all working on ANY tiling (from
  geometry + adjacency): **`left-to-right`** (reading order — rows top→bottom, each left→right), **`spiral`**
  (a true flowing spiral, built as a greedy **wall-following WALK** — from the centre, each step takes the
  unvisited neighbour reached by the sharpest right turn from the current heading, i.e. the classic
  right-hand rule — so it winds outward as one connected +1 path to an edge, consecutive numbers always
  adjacent), **`radial`** (concentric rings out from the centre).
  Pure `src/tiling/numbering.ts`
  ...
  **The scheme is the ONE user-facing tile number everywhere** — the stats label, Inspect "Tile #N", the
  ...
  walkers/ticks. Recipe **schema v9** (additive `numberingScheme`; migrations default old images so they
  reproduce identically). See §9. *(Shipped a first cut — normal/spiral rings + pictogram buttons — then the
  owner refined it the same day to the three geometric schemes + a proper flowing spiral + the dropdown.)*
- **Debug log — write-target highlighting + a pulsing highlight (2026-07-09):** the per-tick decision log now
  shows, for every `put`/`increase`, the **exact tiles it wrote to** — one hoverable sub-row per element of a
  multi-target list (like a move's candidates). **Hover the row** → all written tiles light up; **hover one
  target** (e.g. `B.f1.e3.e3.e2`) → just that tile is pinpointed; a target that resolved **off-grid** shows
  "→ off-grid (no write)" and the row badges **"no target"** when nothing landed — so a stray/typo'd `.`-path
  (the kind that silently writes nowhere) is finally visible at a glance. All debug highlights now **pulse**
  (a gentle sine breath, rAF-driven only while a row is hovered/pinned — zero cost at rest) so they're easy to
  spot on a dense grid, and written tiles get their own **magenta "writes" role** distinct from the orange
  "moved". The write trace grew a `targets: WriteTargetTrace[]` field (resolved per element in `exec.ts`'s
  `applyWrite`); pure mappers `writeTargetGroups` + a `write` case in `statementGroups` (`src/debug/highlights.ts`);
  `drawHighlights` takes a `pulse` factor. Opt-in trace only, so the export path stays byte-identical. See §9.
- **Gallery share + deep links (2026-07-09):** the spotlight view is now **directly linkable** — opening a
  creation puts `#/gallery/<id>` in the URL (still the gallery route via the existing startsWith router; Back
  closes it), and a new **Share button** (the classic three-node share glyph) hands over that link — **native
  share sheet on touch** (coarse pointer), **copy-to-clipboard with a "Copied!" flash on desktop**. A shared
  link opens that exact image's spotlight even when it isn't on the recipient's loaded feed page: a new
  single-creation endpoint **`GET /api/creations/:id`** (`functions/api/creations/[id]/index.ts` + client
  `fetchCreation`) fetches just that one; a dead link shows a friendly "could not be found" rather than hanging.
  Spotlight state moved from local React state onto the hash (`useSpotlightId` via `useSyncExternalStore`;
  `gallerySpotlightHref`/`spotlightIdFromHash` in the router). Pure additions — no new deps, no recipe/schema
  change.
- **Fix — export froze a fill pattern in a centred blob (default `max-steps` too low, 2026-07-09):** a
  single-walker fill pattern (e.g. a `find-lowest-tile` walker, which visits ONE tile per step and never
  splits) was silently capped at ~50 000 tiles no matter how big the export grid — a ~224×224 (√50 000)
  centred blob, because the per-walker **`max-steps`** default was **50 000**, well below the run's
  1 000 000-tick backstop. The walker was retired on its step cap (not the tick cap), and only the TICK cap
  raises the "may be incomplete" (!) badge, so it truncated with no warning. Raised `DEFAULT_SETTINGS.maxSteps`
  to **1 000 000** so it's never the first limit to bind (a walker gains one step per tick, so the equal tick
  cap always stops first — and that IS surfaced); a fill pattern now runs to a natural finish and covers the
  whole export. Not the find-lowest cache or f0/f1 (a cached run == a brute-force run exactly). Headless proof
  on the real export path: a 320×320 knight went from a frozen 229×229 blob to filling 320×320 (97%). Gallery
  recipes unaffected (their ~4 120-tile grids never approach 50 000 steps). See §9. `max-steps = N` still lets
  a program retire a walker early on purpose.
- **Fix — a stale export worker after a redeploy now fails with a "reload" hint, not a cryptic error
  (2026-07-09):** the export helper is a **separate, content-hashed chunk fetched lazily on the first
  export**, so a redeploy (which renames it) 404s an already-open tab → the worker can't start → the log's
  bare `worker.onerror` with an empty event (the intermittent "export worker failed" from §7). Now a
  worker LOAD/START failure (bare `onerror`, or a `new Worker` construction throw from a CSP) rejects with
  `WORKER_UNAVAILABLE_MESSAGE` — an **actionable "Exploroboros was updated, reload (Ctrl+Shift+R) and export
  again"** — flagged `workerUnavailable` (`isWorkerUnavailable`), and the Workspace shows it as a toast and
  **skips the debug-log download** (a reload is the fix, not a bug report). A genuine compute/render throw
  still arrives STRUCTURED (with a stage) and keeps the debug log. **Deliberately NOT a silent main-thread
  fallback** (owner's call — masking the failure was rejected). Owner also weighed the true root fix
  (inline the worker so there's no separate chunk to 404) and chose the message over the ~40 KB-gzip bundle
  cost. `src/export/exportImage.ts` + `Workspace.tsx`; see §9.
- **Fix — Share button no longer resizes on copy; spotlight image gained scroll-wheel zoom (2026-07-10):**
  the Share button used to swap its own label between "Share" and "Copied!", changing its width and
  shifting the rest of the `.spot-actions` row — reported as ugly by the owner. It's now **icon-only at
  all times**; "Copied!" instead floats as a small pill above the button (`.spot-share-tip`, absolutely
  positioned off a new `.spot-share-wrap`) and fades out after the same ~1.8s. Also: hovering the spotlight
  image and using the **scroll wheel now zooms in/out** (1x–5x), centered on the cursor — a native
  (non-React) `wheel` listener on `.spot-image` (`{ passive: false }`, since React's `onWheel` is passive by
  default and can't block page scroll), scaling the `<img>` via `transform: scale()` with a per-event
  `transform-origin` set from the cursor position; resets to 1x when the spotlighted creation changes.
  `src/components/GallerySpotlight.tsx` + `.css`.
- **Fix — the desktop canvas workspace was capped at 94rem, squeezing the canvas to a sliver with both
  side panes open (2026-07-10):** `Canvas.css`'s desktop breakout (`width: min(94rem, calc(100vw - 4rem))`)
  had a rem-based ceiling that bound on any monitor wider than ~1568px, leaving visible dead space on both
  sides while the workspace (two 34rem panes + canvas) squeezed into the capped column — on a wide monitor
  with both a left and right pane open the canvas-pane could shrink to a sliver. Dropped the cap:
  `width: calc(100vw - 2rem)`, so the workspace claims (nearly) the full window on any desktop size. (An
  earlier attempt widened `.app-main`'s own max-width instead — reverted as a no-op: `.canvas-page`'s
  `margin-left: 50%; transform: translateX(-50%)` breakout centers on `.app-main`'s centre regardless of
  its width, since `.app-main` is itself centred on the viewport either way — so the ACTUAL cap was always
  the local `min(94rem, …)`, not the shared `--maxw`.) Mobile (< 64rem) unaffected.
- **DSL path separator changed from `@` to `.` (2026-07-11):** the "read/hop to another tile" separator is now
  a dot everywhere — `visited.e1`, `move e0.e4`, `.tile N`, `exists.f0`, `put [B.e1] = 1` — across the
  traverser DSL, predicates, colorings, the Guide, and all help text. `@` is no longer a DSL character.
  **Lexing stays unambiguous** because the precedence already in both lexers resolves the two collisions: the
  number branch only eats a `.` when a digit follows (so `.5`/`3.5` are numbers but `.e1`/`.tile` are hops),
  and the traverser lexer matches the two-char `..` range before a lone `.` (so `e1..e3` is a range, `e0.e4`
  a hop). Mechanically: the `src/dsl` lexer's `@` token became a `.`-triggered `dot` token; the traverser
  lexer swapped `@` for `.` in its single-char set; the two serializers emit `.`; the in-repo gallery/prototype
  recipes + every DSL example were rewritten. Recipe schema bumped to **v10**. **The one-time production
  rewrite ran successfully** (`tools/migrate-paths.mjs`: dry-run → `--apply` → dry-run confirmed clean, so
  the live gallery's stored DSL text was already `.`-only). **Once that was confirmed, the migration + the
  tool were removed entirely** (same day, 2026-07-11, owner's call): `src/export/recipe.ts` has no `from: 9`
  step any more (a deliberate gap — `migrateRecipe` returns `null` past v9) and `tools/migrate-paths.mjs` is
...
- **Duplicate a coloring rule + a `back` edge keyword (2026-07-12):** two small independent additions. (1) A
  **duplicate button** (copy icon, between the eye and trash in each Coloring-rule row) inserts a deep copy of
  that rule directly below it — pure `withDuplicatedRule` + store `duplicate` in `coloringStore.ts`, new
  `DuplicateButton.tsx` (reuses the shared `.icon-btn` style). (2) **`back`** — the reverse of `straight` — is
  now a first-class edge everywhere `straight` is (`move back`, chains `straight.back`, guards, `.`-paths
  `visited.back`): it exits the straight-through PARTNER of the heading edge (a U-turn), reusing the shape's
  arrival pairing so it crosses the concave wedge cleanly; in `absolute` movement the reference edge is north.
  Added to `EdgeRef` (traverse) + `PathSeg` (dsl) at the parse / serialize / `resolveRef` / `segToEdgeRef` /
  reserved-word sites (TypeScript exhaustiveness caught every switch); walker-free (coloring) contexts fall
  back to default like the other relative hops. Recipe **schema v10 → v11** (additive no-op migration, same
  rationale as v6→v7 — stamps a `back`-using image so an older/cached build refuses it cleanly rather than
  failing to compile the traverser). Guide + Traversers help updated (and a stale `straight -> r1` example
  fixed to the current `straight.r1`).
- **Tutorial mobile bubble fix (2026-07-12):** on phones the anchored speech bubbles rendered off-screen
  horizontally (the JS `placeBubble` set a `right`-placed bubble's `left` to the anchor's right edge with
  no upper clamp, and on mobile every panel is near-full-viewport-width, so the bubble started past the
  edge with nothing pulling it back). Fix: below **64rem** (the app-wide Canvas/Workspace breakpoint), a
  DOM-anchored bubble skips its anchor math entirely and stacks into ONE full-width **banner pinned to the
  bottom** of the viewport (`.tut-banner`) — nothing can go off-screen. Bottom, not top, because 4 of the
  12 steps anchor to the top toolbar (Step/Play) and a top banner would cover the very button being taught.
  `center`/`canvas-top` steps (welcome, finale, the tile-select prompt) are untouched — already
  viewport-relative and on-screen. The banner is the one bubble container that catches its own clicks
  (`pointer-events: auto`, so it's touch-scrollable when tall) and runs the same copy + advance/block-hint
  a catcher does; the inner bubbles stay `pointer-events: none`. Also trimmed ", right inside the real
  canvas" from the Tutorial landing blurb. See §9.
- **Tutorial Chapter 2 "Colorings" + a global ramp-default change (2026-07-12):** the second guided chapter,
  plus a small everywhere-change it leans on. **(Global)** turning a flat colour into a ramp (the `+` in a
  coloring rule) now defaults to fading over **`latest-step` modulo 10** (was `visited` mod 6) — a livelier
  default that animates along a walker's path out of the box (`toRamp` in `ColorField.tsx`). **(Chapter)** an
  11-step walkthrough: it opens with a pre-placed **`straight walker`** (`move straight`) on a blank board,
  has the user open the Coloring pane, add a rule, pick a color, Play (watch the line paint), press `+` to
  make it a fade (now `latest-step`/10), Play again, then — to teach **rule ORDER + opacity** — on Stop it
  adds two branching fill walkers **`softie-A`/`softie-B`** (writing registries A/B) at the top corners, colors
  by two **`latest-step`** fades (the top rule opaque, the second at 50% so it blends) plus an opaque covering
  rule on top, and **plays the run live** so the user watches the board fill; the user deletes the covering
  rule to reveal the two overlapping fades, then a fireworks finale. This needed a **new tutorial seam**: a per-step **`SceneSetup`** (open a pane / set traverser
  defs / place seeds with an explicit heading / set the coloring / stop / **pre-fill via `runToCompletion`**),
  applied once when a step becomes active, plus four new `TutorialSignals` (`runEnded`, `coloringRuleCount`,
  `firstRuleColorHex`, `firstRuleIsRamp`) and four new `data-tut` anchors on the Coloring pane (the pane itself,
  `add-coloring-rule`, `rule-swatch`, `add-color`). Chapter-level `coloring`/`forceTraverserName`/`stopAtStep`
  became optional (this chapter drives coloring through step setups and finishes naturally). **Note on rule
  order:** this app composites rules **top→bottom with the LAST rule on top**, so the covering rule is the
  BOTTOM one in the list (the owner's "above them" reads as the inverse — the bubble teaches the real model).
  See §9. *(Verified headlessly end-to-end; the on-canvas colours + fireworks are the owner's device pass.)*
  **Refined the same session (pre-first-commit, 2026-07-12):** (1) every bubble now shows a small muted
  **"N/total" step badge** top-right (`.tut-bubble-num`, `var(--muted)`, 0.72rem — so a bubble is easy to
  refer to as "step 3/11"), fed by the controller's `index`/`total`, on **all** chapters. (2) The welcome
  step's `SceneSetup` gained **`editTraverser`** (a def NAME): it opens that walker in the Traversers
  full-pane **editor** so its `move straight` code is visible while the next bubble talks about it —
  `TraversersPane` takes a `forceEditName` prop (an effect opens the matching def once it's in the store);
  Workspace holds `tutEditDef`, set from the scene setup and persisted across steps. (3) The **pick-color /
  add-color / play-2** bubbles now **anchor to the whole Coloring pane** (`{tut:'coloring'}`, placement
  right) instead of the small control they used to cover — the `hole`/ring still marks the actual target
  (swatch / `+` / Play), so the bubble emanates from the pane without hiding the rule it describes. (4) UI
  copy switched to American **"color"** spelling (bubbles + the chapter blurb). (5) The **ordering** (last
  teaching) step was reworked: the straight walker is renamed **`forest_gump`**; the two reveal fades now
  fade over **`latest-step`** (not the registry value), the **top rule fully opaque** and the second at 50%
  (so it blends where the walkers overlap); and it **plays the run live** via a new **`SceneSetup.play`**
...
- **Fix — dragging in the colour picker lagged the UI (now commits on release, 2026-07-13):** the coloring
  colour swatch is a native `<input type="color">`, and React's `onChange` on it binds to the DOM **`input`**
  event, which **streams continuously** while you drag inside the OS colour dialog. Each intermediate colour
  was pushed to the coloring store → re-`colorize` the WHOLE tiling + redraw the canvas, many times a second
  → stutter on a dense grid. Now the swatch commits **only** on the native **`change`** event (fired when the
  picker is dismissed), so the tiling recolours exactly once, on release — the behaviour the owner asked for.
  Implementation (`src/components/ColorPicker.tsx`): the input is **uncontrolled** (`defaultValue`) so React
  doesn't fight the live drag, a `change` listener is wired via a ref (React's `onChange` = the `input`
  event, so it can't be used for commit-only), and an effect syncs the swatch's DOM value when the colour is
...
- **Canvas background toggle — invert the plane to a dark scheme (2026-07-15):** a new **background**
  control in the **canvas settings** section (foot of Inspect), a two-option `SegmentedControl` (**white** /
  **black**) beside `tile display`. **white** (default) = the original white tile fill + black grid lines;
  **black** flips the LIVE canvas to a black plane with white grid lines. Implemented as a small `INVERTED`
  partial palette in `TilingCanvas.tsx` (`tile` → `#000`, `edge` → `#fff`, plus the number label `num` and
  the traverser arrows `traverser`/`traverserAuto` — all tuned dark for the white plane — flipped light for
  legibility; the themed accents visited/selection read fine on either), spread over the base palette inside
  the existing CSS-variable read effect (now keyed on the new `inverted` prop so a toggle re-reads + redraws).
  A **live-view preference only** — like `displayMode` it's ephemeral React state (not persisted, not in the
  recipe); the **export dialog keeps its own separate background control** (black/white/transparent), so
  exports are unaffected. `Workspace.tsx` owns `invertColors` state + the control; `TilingCanvas` gained an
  `inverted?: boolean` prop. No new deps, no recipe/schema change. See §9.
- **Traverser editor — compile-error badge on the text box (2026-07-15):** when the program in the
  maximised Traversers editor doesn't compile, a small orange **"error"** pill now sits in the **top-right
  corner of the code textarea** (shown only while `!result.ok`), complementing the red badge the list
  already shows on a non-compiling definition. Purely a visual at-a-glance marker: it's `aria-hidden` (the
  always-visible status line below the box still carries the message + its `role="alert"`) and
  `pointer-events: none` (follows the swatch-gutter precedent, so it never intercepts clicks/typing in the
  textarea beneath it), styled to match the list's `.pred-badge-err`. `TraverserEditor` already computed the
  live `compileProgram` result, so this is one guarded `<span>` inside the existing `position: relative`
  `.trav-edit-textwrap` + a `.trav-edit-badge` rule. No new deps, no recipe/schema change. `TraversersPane.tsx`
  + `.css`.
- **Initial-state presets (Corners / Midpoints) + a guard-aware blob anchor (2026-07-16):** two more
  entries in the Initial-state pane's presets dropdown — **Corners** (a size-1 blob of `t1` in each of the
  four corners) and **Midpoints** (one at the middle of each outer edge) — each aimed **inward** using the
  square grid's heading numbering (0 = up, 1 = right, 2 = down, 3 = left; top corners/edge aim down, bottom
  aim up, left aims right, right aims left). A true diagonal isn't an edge on a square so Corners use the
  nearest straight direction; on non-square tilings the heading is approximate (owner's call — matches the
  existing Edges/Cross presets). Pure data in `src/data/initialStatePresets.ts`. **Plus a behaviour fix a
  guarded `blob` needed to be useful:** `blobTiles` gained an optional `accept` predicate, and
  `resolveInitialState` passes the statement's guard into it, so a guarded blob **anchors on the nearest
  tile that PASSES the guard** (searching all tiles nearest-first) instead of only testing the single
  nearest tile and giving up. So `auto-place blob {t1, 0, 0, 1, 0} if tile-type == hexagon` snaps to the
  nearest hexagon to the corner even when the exact corner tile is a different shape — it does its best to
  place at least one, and lands nothing only if NO tile matches anywhere (a shape the tiling lacks). Rings
  are still returned unfiltered (the caller's guard filters the set, so a blob still reaches accepting tiles
  through non-accepting ones). No `accept` = unchanged; `line` unchanged. No recipe/schema change (presets
  are text; the resolver runs live + in export identically). Guide + pane help updated.
- **Fix — a tile showed TWO "orientation" fields in Inspect (a naming collision, 2026-07-16):** the owner
  hit a directive `if orientation != 0 and orientation.target != 0 always forbid move` on rhombille that
  didn't block an (apparent) orientation-1 → orientation-2 move. Root cause was **not** the engine — traced
  headlessly, the directive is correct (a walker whose real DSL `orientation` is 1 IS blocked from every
  orientation-2 destination). The confusion was that Inspect showed the word "orientation" **twice** with
  different values: the geometry-derived **DSL `orientation` attribute** (what rules read; portable across
  tilings, `src/tiling/orientation.ts`) AND a per-tiling **lattice discriminator coordinate** that the
  rhombille + triangular generators happened to also label "orientation" — and the two number the variants
  differently (rhombille lattice `0/1/2` = DSL orientation `2/0/1`), so the owner read one and the rule read
  the other. Fix: renamed the colliding lattice labels (**rhombille → `face`**, **triangular → `facing`**;
  `latticeLabels` is display-only — `coordinate[n]` reads lattice by INDEX, so tile ids / recipes / gallery
...
- **Computed (expression) numbered references + `tN` shorthand (2026-07-20):** every numbered reference —
  `eN`/`rN`/`lN`/`fN`/`tN` — now also accepts a parenthesized **numeric expression** in the number slot,
  resolved to an integer at eval time (rounded to nearest): `move r(steps % 2)`, `e(orientation +
  orientation.e2)`, `l(A + B.e0)`, `f([A, B, visited.r2.r2]:max)`, `.e(orientation)`, `.t(steps + 1)`. The
  literal forms are unchanged. Two companion changes: **`tile N` → `tN`** (shortened to match the family; the
  old `tile N` still parses as a legacy alias and re-serializes to `tN`), and **`r0`/`l0` are now valid and
  both mean `straight`** (a 0-turn). `tN`/`t(expr)` is also a move-chain **base** like `fN` (`move t5` jumps
  to the absolute tile #5, `move t(steps)` / `move t5.e0`) — a jump keeps the walker's current heading. The
  amount reuses the shared `Expr` grammar (parse delegates the parens to `parseExpr`; `evalNumber` at
  resolution): the "number" slot widened to `EdgeAmount = number | Expr` on `PathSeg` + `EdgeRef`, an
  `AmountEval` callback threads per-hop context through `resolveRef`/`resolveChain`/`walkChain`, and the four
  `target.ts` analysis walkers + the two lexers-free serializers recurse into amounts (so a `find-lowest`
  predicate correctly rejects `e(steps)` as walker-dependent, a guard reading `e(visited.target)` gates
  per-candidate, etc.). Walker-free contexts (coloring, preview) evaluate absolute amounts and read walker
...
| 2026-07-08 | **Board numbering (left-to-right / spiral / radial) + `find-lowest/highest-tile` + "canvas settings" pane.** A user-chosen tile **numbering** — **left-to-right** (reading order), **spiral** (a greedy wall-following WALK from the centre — each step turns the sharpest right onto an unvisited neighbour, winding out as one +1 path to an edge), or **radial** (concentric rings) — is the ONE user-facing tile number everywhere: the stats label, Inspect "Tile #N", the `tile-number` DSL attribute (coloring + traverser + init-state guards), `.tile N` addressing, and the new searches; raw generation order is never surfaced. All three work on any tiling from geometry + adjacency (`src/tiling/numbering.ts`). Picked from a **dropdown** in a new **"canvas settings"** section at the foot of Inspect (always visible), which also hosts the **display-mode** control moved out of the toolbar. New **`find-lowest-tile`/`find-highest-tile <pred>`** — a global scan for the lowest/highest-**numbered** matching tile → `fN` (`move f0`, `exists.f0` all work) — backed by an efficient per-query **bookmark cache** (`src/traverse/lang/findLowest.ts`; forward-resume, nudge back only on a *changed* tile below it, walker-free condition compile-enforced). Recipe **schema v9** (additive `numberingScheme`; migrations default old images so they reproduce identically — no recipe uses `tile-number`/`.tile`, so nothing existing changed). | ✅ yes | owner asked for the numbering options + the canvas-settings section + a "go to the lowest tile of a predicate" structure with a smart cache ("nudge back on *change*, not just decrease" — implemented as written). Shipped a first cut (normal/spiral rings + pictogram buttons), then the owner refined it: coloring by `tile-number` must follow the scheme (fixed → everything user-facing now does), "spiral should be a path that winds +1 around and around to the edge" (added a true spiral; the old rings became **radial**), "left-to-right for hexagons is wrong — top to bottom first" (made it a geometric reading order), and "remove the buttons, use a dropdown". A first pass built `spiral` as sorted concentric rings, which the owner found stranded tiles ("even worse now") and then DEFINED precisely — "start in the middle, move the sharpest right you can without going backwards" — so `spiral` was rebuilt as exactly that greedy wall-following WALK (a 7×7 dump winds 0-at-centre → 48-at-corner, every step edge-adjacent — a clean clockwise spiral). Backed by build / lint / **879 tests** (numbering.test: left-to-right reads the top row L→R on square + top→bottom/left→right on HEX; radial rings monotone-by-distance; spiral centre-first + ends-on-boundary + consecutive-numbers-edge-adjacent walk + distinct-from-radial; find-lowest cache forward/nudge-back/neighbour/global; v8→v9 + v7→v8 migrations; colorize + traverser `tile-number`/`.tile`-follow-scheme guards). The corrected spiral was served on this worktree's preview (5697) with a 7×7 dump shown (0-at-centre winding to 48-at-corner, every step adjacent); owner reviewed and said "excellent please commit". | — |
| 2026-07-09 | **Debug log — write-target highlighting + a pulsing highlight.** The per-tick decision log now expands each `put`/`increase` into one hoverable sub-row per resolved target (like a move's candidates): **hover the row** lights every tile it wrote to; **hover one target** (`B.f1.e3.e3.e2`) pinpoints that single tile; a target that resolved **off-grid** reads "→ off-grid (no write)" and the row badges **"no target"** when nothing landed — so a stray/typo'd `.`-path that silently writes nowhere is visible at a glance (the exact class of bug behind the owner's Black/Red-knight `B.f0` / in-block-`f0` confusion). All debug highlights now **pulse** (a gentle rAF sine breath, live only while a row is hovered/pinned — zero cost at rest) to stand out on a dense grid; written tiles get their own magenta "writes" role, distinct from orange "moved". Opt-in trace only → export path byte-identical. Files: `trace.ts` (`WriteTargetTrace` + `targets` on the write trace), `exec.ts` (`applyWrite` records each resolved target), `highlights.ts` (`writeTargetGroups` + a `write` case), `TilingCanvas.tsx` (`write` role + `pulse` factor + the pulse rAF), `DebugPane.tsx`/`.css` (target sub-rows, "writes" legend, pulse note). | ✅ yes | owner asked for the write-target highlighting + a slight pulsing effect; built both, owner reviewed on this worktree's preview (5553, footer-labelled) and said "please commit". Backed by build / lint / **882 tests** (5 new/expanded: write trace records resolved targets + off-grid → null; `statementGroups` write case + `writeTargetGroups` single-target + off-grid/walker lights none) + served-source confirmation the branch was live. The pure mappers + trace recording are test-covered; the interactive hover feel + the pulse animation are a visual judgement left to the owner's device (per the §9 headless-preview caveats). | — |
| 2026-07-09 | **Gallery share button + direct links to a spotlight image.** The spotlight view is deep-linkable (`#/gallery/<id>`, state on the URL hash via `useSpotlightId`/`useSyncExternalStore`; Back closes it); a **Share button** (the classic three-node share icon) **copies that link on desktop** ("Copied!" flash) and **opens the native share sheet on touch** (coarse pointer). A shared link opens that exact image even when it's off the recipient's loaded feed page — new single-creation endpoint **`GET /api/creations/:id`** (`functions/api/creations/[id]/index.ts` + client `fetchCreation`); a missing id shows a friendly "could not be found". No new deps, no schema bump. | ✅ yes | owner reviewed on this worktree's own seeded gallery backend (`wrangler pages dev`, port 9188, footer-labelled) — Share button + icon render, a deep link opens the right spotlight, a copied link reopens it — and said "looks good, please commit". Backed by build / lint / **891 tests** (12 new: 5 Gallery deep-link/fetch/error/close/card-open tests, 5 router href/parse round-trip tests, 2 `fetchCreation` success/404 tests) + live checks on the seeded backend (list; `GET /api/creations/:id` 200 + 404; image route; deep-link opens the spotlight with the Share button; missing-id renders the error; the served bundle carries the new strings). | — |
| 2026-07-09 | **Fix — export froze a single-walker fill pattern in a centred ~224×224 blob (default `max-steps` too low).** The owner's `find-lowest-tile` knight visits ONE tile per step and never splits, so the per-walker **`max-steps`** default of **50 000** silently capped it at ~50 000 tiles (√50 000 ≈ 224) — a centred blob, the SAME size whether exported at 250×250 or 500×500. It was retired on the STEP cap, which (unlike the run-wide tick cap) raises no "may be incomplete" badge, so the truncation was invisible. A `move straight` walker was unaffected (only ~N/2 steps to the edge). Raised `DEFAULT_SETTINGS.maxSteps` (`src/traverse/lang/types.ts`) 50 000 → **1 000 000**, matching the run's 1 000 000-tick backstop so it's never the first limit to bind (steps ≤ ticks always ⇒ the equal tick cap stops first, and that DOES surface). NOT the find-lowest cache / f0 / f1: a cached run equalled a brute-force (fresh-cache-per-tick) run tile-for-tile. | ✅ yes (headless, via feedback loop) | owner reported the fixed export size (~250 field regardless of grid) and set up a "I can't test, you fix + verify" loop since a far-out export is slow to generate by hand. Root-caused with a headless probe on the REAL export path (`runToCompletion` + the compiled knight + real numbering): before, N=240 and N=320 both froze at 229×229 with thousands of matching tiles still free and `hitCap=false` (⇒ a walker DROP, not the tick cap) at exactly 50 001 steps; after, N=320 fills 320×320 (97%) and stops naturally. Cache exonerated by cached==brute at N=80. Backed by build / lint / **894 tests** (`parse.test` default updated; gallery grow-check unchanged — their ≤4 120-tile grids never approach the cap; count is post-rebase onto main's gallery-share work). Owner reviewed the plain-language diagnosis + headless evidence and said "good please commit"; the slow on-device export was not hand-watched (headless run of the identical export function is the evidence). | — |
| 2026-07-09 | **Fix — a stale export worker after a redeploy fails with an actionable "reload" message (was the cryptic "export worker failed").** Diagnosed the owner's deployed-site export-failure debug log: a bare `worker.onerror` with an empty event + no stage = the worker MODULE didn't load. Root cause = the helper is a separate content-hashed chunk fetched lazily on the first export, so a redeploy that renamed it 404s an already-open tab (explained the hash/lazy-load mechanism to the owner). The compute path was exonerated (ran the EXACT recipe from the log through `computeExport` headlessly — no throw) and the worker's whole import graph is DOM-clean (grep) so it's not a code crash. Owner **rejected a silent main-thread fallback**, and (given the ~40 KB-gzip cost of the true root fix — inlining the worker so there's no separate chunk) **chose the reload message**. New `WORKER_UNAVAILABLE_MESSAGE` + `workerUnavailable` flag + `isWorkerUnavailable` (`src/export/exportImage.ts`): a bare `onerror` OR a `new Worker` construction throw now rejects with the actionable message; `Workspace.tsx`'s export `.catch` shows it as a toast and skips the debug log. A genuine (structured, staged) compute/render failure is untouched — still logs. | ✅ yes (headless — see note) | owner picked "fail with a reload message" over inlining; implemented. Backed by build / lint / **897 tests** (3 new in `exportImage.test.ts`: a worker-load failure is flagged `isWorkerUnavailable` + carries the reload message; a STAGED compute/render failure is NOT flagged so it still gets a debug log; plain error / abort are not flagged). The pure classification is unit-tested; the end-to-end worker-fail→toast wiring can't be exercised (jsdom has no real Worker; the dev server loads the worker from source so it never 404s), so that hop rests on reasoning + the unit test — it proves out the next time a redeploy strands a tab. Owner's live tab was separately unblocked by a hard reload. | — |
| 2026-07-10 | **"Load prototype ports" now serves a RANDOM tiling-agnostic fractal traverser (a palette of 51).** The debug button at the foot of the Traversers pane no longer adds the single gasket — it adds ONE definition picked at random, weighted by a per-entry `beauty` (1–3) so the showcase ones surface most (click again to shuffle). `src/data/prototypePorts.ts` became a documented PALETTE of **51** walkers across **12 distinct MECHANISM families** — XOR/Rule-90 gaskets, timed branching (veins/ferns), turmites (Langton-ant), self-avoiding mazes, structural routing (by step / tile-number / edge-count / orientation), walker-state machines (`P` + `update`), chained `.`-hops, directive phases (`reset`), list reducers (`:xor`/`:any`), search-based (`find-tile` / `find-lowest-tile` / `exists.f0`), and registry cellular automata — built ONLY from universal mechanisms (no `tile-type`/octagon/wedge names), so each grows something on ANY tiling, and between them they exercise every traverser DSL feature. A placed walker inherits the def's header (`max-split`/heading/movement), so the split-fans work from one centre placement (verified via `makeSeed` in `Workspace.tsx`). New `pickRandomPort()` (weighted); button label kept, tooltip updated. Colour stays separate (the Coloring pane reads the `visited`/`[A]` data). | ⚠️ partial — visual is the owner's, by their choice | owner asked for ~50 varied tiling-agnostic fractal traversers, the button to serve a random likely-beautiful one, and explicitly to NOT visually test them ("i will test them") — just to be pointed at the file to copy/adjust. So on-device beauty is the owner's to judge; I verified only the finite correctness — build / lint / **901 tests** (6 in `prototypePorts.test.ts`: all 51 compile against an empty predicate map, names unique + non-reserved, every `beauty` in 1–3, `pickRandomPort` always returns a member, the gasket still ports its XOR-unique fork) + served-source confirmation the palette + random button are live on this worktree's preview (5200). Owner reviewed the plain-language summary and said "good please commit". | — |
| 2026-07-10 | **Fix — Share button resized on copy; spotlight image gained scroll-wheel zoom.** The Share button swapped its own label ("Share" ↔ "Copied!"), changing its width and shifting the `.spot-actions` row — the owner called it ugly. Now icon-only always; "Copied!" floats as a small pill above the button (`.spot-share-tip`) and fades after ~1.8s, with no layout shift. Also: scroll-wheel over the spotlight image now zooms 1x–5x, centered on the cursor (a native `wheel` listener, since React's `onWheel` is passive and can't block page scroll). `src/components/GallerySpotlight.tsx` + `.css`; no schema/deps change. | ✅ yes | owner reported the resize bug + asked for scroll-zoom; this preview tab came up `document.hidden` from the start (§9 — screenshots/bounding-boxes unusable), so verification drove the actual button clicks and synthetic `WheelEvent`s and read the resulting DOM/state directly: button markup stays icon-only across both states, a stubbed clipboard call resolves and shows the floating "Copied!" pill (confirmed once an artificial delay ruled out the pill just having already auto-hidden by the time of the round-trip), and dispatched wheel events zoom in, zoom out, clamp at 1x, and toggle `is-zoomed` correctly, with `dispatchEvent` returning `false` (preventDefault honoured). Backed by build / lint / **897 tests** on this worktree's own seeded gallery backend (`wrangler pages dev`, port 8986, footer-labelled). Owner said "good please commit". | — |
| 2026-07-10 | **Fix — desktop canvas workspace capped at 94rem, crowding the canvas with both panes open.** `Canvas.css`'s desktop breakout had a `min(94rem, calc(100vw - 4rem))` ceiling that bound on any monitor wider than ~1568px — visible dead space on both sides while the workspace (two 34rem panes + canvas) squeezed into the capped column. Dropped the cap to `calc(100vw - 2rem)`; the workspace now claims (nearly) the full window on any desktop size. Mobile unaffected. | ✅ yes | owner reported the canvas shrinking to a sliver with both panes open on a wide monitor; root-caused via precise `getBoundingClientRect` measurements on this worktree's preview rather than screenshots (this preview tab is `document.hidden`, per §9): at 1920px wide the canvas-pane went from 339px → 723px with a left + right pane both open (the worst case described); at 1440px wide 212px → 243px (a smaller gain there, since the 94rem cap wasn't yet binding at that width — the remaining squeeze at laptop sizes is just the two fixed 34rem panes, a separate concern flagged to the owner). Confirmed mobile (390px) layout and `scrollWidth` unchanged. An initial attempt widened `.app-main`'s max-width instead; measurement showed it was a no-op (the breakout centers on `.app-main`'s own centre regardless of its width) and was reverted before committing. Backed by build / lint / **901 tests**. Owner said "good lets try that for a while, please commit". | — |
...
| 2026-07-12 | **Duplicate a coloring rule.** A **duplicate button** (copy icon) sits in each Coloring-rule row **between the eye and the trash**; clicking it inserts a deep copy of that rule **directly below it** — same predicate / colour (ramps deep-cloned) / opacity / on-off state, fresh id — so the copy is tweakable independently. Pure `withDuplicatedRule` + store `duplicate` (`coloringStore.ts`); new `DuplicateButton.tsx` reusing the shared `.icon-btn` style. | ✅ yes | owner said "excellent please commit"; verified live in the running app on this worktree's preview (**5386**, footer-labelled): added a rule → set it to `Has_A` → clicked duplicate → **two identical `Has_A` rows**, each with reorder/eye/duplicate/delete in that order. Backed by build / lint / **999 tests** (pure `withDuplicatedRule` incl. inserted-directly-after + unknown-id no-op + deep-clone of ramp stops; a `ColoringPane` click test that duplicates a row). The icon's on-screen look is the owner's device call (the preview tab was `document.hidden`, so `preview_screenshot` timed out per §9 — DOM reads confirmed placement + behaviour). | — |
...
| 2026-07-15 | **Canvas background toggle — invert the plane to a dark scheme.** A **background** two-option `SegmentedControl` (**white** / **black**) in the **canvas settings** section (foot of Inspect), beside `tile display`. **black** flips the LIVE canvas to a black plane with white grid lines (+ light tile numbers / traverser arrows for legibility); **white** is the default original look. A small `INVERTED` partial palette in `TilingCanvas.tsx` (`tile`→`#000`, `edge`→`#fff`, `num`/`traverser`/`traverserAuto` flipped light) spread over the base palette inside the CSS-var read effect, now keyed on a new `inverted?: boolean` prop so a toggle re-reads + redraws. A **live-view preference only** — ephemeral like `displayMode` (not persisted, not in the recipe); the export dialog keeps its own separate background control, so exports are unaffected. No new deps, no recipe/schema change. | ✅ yes | owner reviewed on this worktree's preview (5553, footer-labelled) and said "looks good please commit"; backed by build / lint / **1041 tests** + served-source confirmation the branch is live on 5553 (the `INVERTED` palette with `#000`/`#fff`, the `inverted ? INVERTED : null` apply, the `invertColors` state + `inverted={invertColors}` prop pass, and the "canvas background" control). The pixels themselves (black bg + white edges rendering) were the owner's device pass — this preview tab is `document.hidden` (§9), so the Konva stage never mounts for a screenshot. | — |
...
| 2026-07-21 | **Export dialog — grid/resolution made independent + ↑/↓ aspect-transfer arrows.** Owner's redesign of the dialog's coupled size controls. The three views — Resolution (output px), Grid (tile counts), Pixels-per-tile (scalar bridge) — now follow: editing the **resolution** re-derives px (grid held); editing **px** re-derives the grid (resolution held); editing the **grid** re-derives px (resolution held) — so grid and resolution never push each other around (previously editing the resolution moved the grid). Resolution is the anchor: it moves ONLY on a direct edit or the new ↓ arrow. Two **↑/↓ arrow buttons** (styled like the chain-locks, a horizontal row centred over the input column between the Grid and Resolution blocks) transfer aspect ratio: **↓** reshapes the resolution to the grid's aspect (longer edge preserved) so a hand-tuned grid fills the PNG with no letterbox; **↑** reshapes the grid to the resolution's aspect. Lock + arrow buttons shrunk to 80% (1.7→1.36rem). All the interdependency math extracted to the pure `src/export/exportControls.ts`; the component is now a thin wrapper. No recipe/schema change (dialog-only). §9 ExportMenu note + help text updated. | ✅ yes | owner iterated on this worktree's preview (5553, footer `cool-cray-0d53f1`) across two rounds — first the independence model + arrows, then "put the arrows on a horizontal row centred between the input fields" + "make the lock and arrow buttons 80% size" — then said "looks good please commit". Backed by build / lint / **1147 tests** (15 in `exportControls.test.ts` — every edit rule + both arrows + the anchor invariant + clamping; 4 in `ExportMenu.test.tsx` — the arrows render in the right DOM order, ↓ makes a 10×80 grid → a 256×2048 resolution with the grid untouched, ↑ makes a 1920×1080 resolution → a ~16:9 grid untouched, editing the resolution moves px not the grid) + a live in-runtime check on 5553 that dynamic-imported the app's OWN `exportControls` and confirmed all five behaviours in the real Vite bundle + a served-CSS check that the horizontal-row/centred layout + 80% size are live. The on-screen look/feel of the arrows was the owner's device pass — this preview tab is `document.hidden` (§9), so React clicks don't fire + `screenshot` times out headlessly. | — |
...
- [ ] **Generic tiling render + data model** *(§4.3)*
  ...
  - [x] Tile numbering as a canvas control — a dropdown in the **canvas settings** section (Inspect) with
    three geometric schemes: **left-to-right** (reading order), **spiral** (a true winding spiral from the
    centre), **radial** (concentric rings). It's the ONE user-facing number everywhere (stats label, Inspect,
    `tile-number` attribute, `.tile N`, `find-lowest/highest-tile`); generation order is never surfaced. Pure
    `src/tiling/numbering.ts`; recipe schema v9 *(2026-07-08)*
  - [x] Canvas background toggle — a **background** two-option control (**white** / **black**) in the
    **canvas settings** section (Inspect), beside `tile display`; **black** inverts the LIVE canvas to a
    black plane with white grid lines (+ light numbers/arrows). A `TilingCanvas` `inverted` prop → an
    `INVERTED` partial palette in the CSS-var read effect; ephemeral view preference (not persisted, not in
    the recipe — export keeps its own separate background control). *(built 2026-07-15; owner to verify)*
...
- [x] **Tile-predicate DSL + Predicate pane + Coloring pane** *(§5)* — pure `src/dsl/` engine
  ...
  - [x] **Duplicate a coloring rule** — a copy-icon button in each rule row (between the eye and the trash)
    inserts a deep copy of the rule directly below it (fresh id; ramps deep-cloned). Pure `withDuplicatedRule`
    + store `duplicate` (`coloringStore.ts`); new `DuplicateButton.tsx` *(verified 2026-07-12)*
...
- [ ] **Port the traverse engine** *(§5)* — reuses the predicate DSL
  ...
  - [x] **Computed (expression) numbered references + `tN` shorthand** — `eN`/`rN`/`lN`/`fN`/`tN` accept a
    parenthesized numeric expression in the number slot (`move r(steps % 2)`, `e(orientation)`,
    `f([A, B]:max)`, `.t(steps + 1)`), resolved to a rounded integer at eval time; literal forms unchanged.
    `tile N` → `tN` (legacy still parses); `r0`/`l0` = `straight`; `tN`/`t(expr)` is a move base like `fN`
    (`move t5`). Number slot = `EdgeAmount = number | Expr`; `AmountEval` threads per-hop context; `target.ts`
    analysis + serializers recurse into amounts; ranges stay literal. Recipe schema v13→v14 (additive)
    *(verified 2026-07-20)*
  - [x] **Initial-state DSL (its own pane)** — `auto-place` moved OUT of the traverser DSL into a right-dock
    **Initial state** pane that seeds the whole starting state: traversers **+** per-tile registries
    ...
    - [x] **Corners / Midpoints presets + guard-aware blob anchor** — two more presets (a size-1 `t1` blob
      in each corner / at each outer-edge midpoint, aimed inward via square-grid headings). And a guarded
      `blob` now **anchors on the nearest tile that passes the guard** (`blobTiles` optional `accept`, fed the
      statement's guard by `resolveInitialState`), so `blob {…} if tile-type == hexagon` snaps to the nearest
      hexagon instead of giving up when the exact nearest tile is a different shape; nothing lands only if no
      tile matches at all *(verified 2026-07-16)*
  ...
  - [x] Prototype-port loader (debug) — the "Load prototype ports" button at the foot of the Traversers
    pane adds ONE ready-made fractal traverser picked at RANDOM (traversal only; colour stays separate),
    weighted by a per-entry `beauty` (1–3) so the showcase ones surface most; click again to shuffle.
    `src/data/prototypePorts.ts` is now a documented **PALETTE of 51** tiling-agnostic walkers across 12
    distinct MECHANISM families (XOR/Rule-90 gaskets, timed branching, turmites, mazes, structural routing,
    walker-state, chained hops, directive phases, list reducers, search-based, registry CA) — built only
    from universal mechanisms (no `tile-type`/octagon/wedge), so each grows something on ANY tiling, and
    between them they exercise every traverser DSL feature. `pickRandomPort()` does the weighted pick. To
    add one, append a `def(name, beauty, [lines])` *(started as the gasket-only loader 2026-06-28,
    `af6a65b`; grew to the random 51-traverser palette 2026-07-10)*
...
- [x] **Reopen from PNG** *(verified 2026-06-28, `a1e6d0b`)* — `Workspace.loadRecipe(recipe)`
  ...
  - [x] **Community gallery — upload + browse** *(done 2026-07-04, `80b2d64`)* — anyone shares an export to a
    ...
    - [x] **Share + direct links to a spotlight image** — the spotlight is deep-linkable (`#/gallery/<id>`,
      state on the hash via `useSpotlightId`); a **Share button** (classic three-node icon) copies the link on
...
- [x] **Debug features + a run log** — a per-tick traverser **decision log** (Debug pane, behind a
  ...
  - [x] **Write-target highlighting + pulsing highlights** — the decision log expands each `put`/`increase`
    into one hoverable sub-row per resolved target: hover the row → every written tile lights up; hover one
    target (`B.f1.e3.e3.e2`) → that tile alone; an off-grid target reads "off-grid (no write)" and the row
    badges "no target" when nothing landed (the tell for a stray/typo'd `.`-path). All debug highlights now
...
- [ ] **Tutorial system** — a guided, simplified take on the real Canvas (§6). The reusable
  ...
  - [x] **Chapter 2 — "Colorings"** — a pre-placed `straight walker`, then: open Coloring → Add rule → pick
    a colour → Play (watch the line paint) → `+` to make it a fade (now `latest-step`/10) → Play → on Stop it
    adds two registry-writing fill walkers (`softie-A`/`softie-B`) + registry fades at 50% + an opaque cover,
    pre-run off-screen → delete the covering rule to reveal the overlapping fades → fireworks. Introduced the
    per-step **`SceneSetup`** seam + four new signals + Coloring-pane anchors (§6/§9) *(2026-07-12)*
  - [ ] More chapters as the app grows (initial-state, export/share, predicates). Each is a new
    `TutorialScript` in `src/tutorial/script.ts` + a `TUTORIAL_CHAPTERS` entry (+ any new `data-tut` anchors
    or `SceneSetup` beats).
...
- `src/components/{ExportMenu,ExportStrip,ImageViewer}.tsx` (+ `.css`) — the export UI (drives `src/export/`).
  ...
  fire-and-forget; it has a "?" explainer for the concept). **The three coupled size views — Resolution
  (output px), Grid (tile counts), Pixels-per-tile (a scalar bridge ≈ res ÷ grid) — and the edit rules that
  tie them live in the pure, unit-tested `src/export/exportControls.ts`.** Resolution is the ANCHOR: it moves
  ONLY on a direct edit OR the ↓ arrow. Editing the resolution re-derives px (grid held); editing px re-derives
  the grid (resolution held); editing the grid re-derives px (resolution held) — so grid and resolution never
  push each other around (they're otherwise INDEPENDENT, may carry different aspects), and `~` marks whichever
  readout is currently derived. Each block keeps its own chain-lock (its W:H proportion). The **↑/↓ transfer
  arrows** — a horizontal row centred over the input column between the two blocks, styled like the locks —
  copy an aspect ratio across: **↓** reshapes the resolution to the grid's aspect (longer edge preserved) so a
  hand-tuned grid fills the PNG with no letterbox; **↑** reshapes the grid to the resolution's aspect. The
  square tiling honours both grid counts directly (rows×cols); every other tiling takes ONE scalar count, so
  `buildTiling` builds its square patch at the LONGER axis and **crops it to the requested w:h rectangle**
  (`cropTilingToAspect`, see the buildTiling note) so a lopsided export still FILLS the frame instead of a
  square patch adrift in the middle. `ExportStrip` is the
...
- `src/export/` — the **pure, isomorphic image-export core** (no React/Konva), public API via
  ...
  no bounds, so the debug log can compute the target size without a tiling), `exportControls.ts` (the Export
  dialog's pure resolution/grid/pixels-per-tile interdependency math + the ↑/↓ aspect-ratio transfer — used
  only by `ExportMenu`, see its §9 note), `recipe.ts` (the serialisable
  ...
  - **Recipe versioning (so images survive app updates).** The recipe carries `schemaVersion` (the
    ...
    field for the board numbering; old images default to the generation-order reading, matters for
    reproduction because the scheme drives the `tile-number` attribute, `.tile N`, and `find-lowest/highest`;
    **+ v8→v9 (2026-07-08), a VALUE remap** — the scheme names/meanings were revised (`src/tiling/numbering.ts`):
    the old `spiral` was really concentric rings → **`radial`**; the old default `normal` (generation order)
    → the geometric **`left-to-right`** reading order; `spiral` now means a true winding spiral. The migration
    maps `spiral`→`radial`, else→`left-to-right`, so a v8 image reproduces the SAME numbering it recorded.
    **v9→v10 (the DSL path separator `@`→`.`) is a DELIBERATE GAP in the chain, not a migration** — the
    owner ran the one-time production rewrite (`tools/migrate-paths.mjs`: dry-run → `--apply` → dry-run
    confirmed clean, so the live gallery's stored DSL text was already `.`-only), then asked for the
    migration AND the tool to be removed entirely (2026-07-11, same day). `MIGRATIONS` has no `from: 9`
    step, so `migrateRecipe` returns `null` for any recipe below v10 and `parseRecipe` reports
    `'unsupported'` — instead of trying to compile leftover `@`-syntax DSL text (which no longer lexes).
    **This is intentional and owner-approved, not a bug:** any pre-change exported PNG, or any gallery row
    whose stored text wasn't already rewritten by the time the tool ran, now fails to open outright rather
    than migrating. **v10 → v11 (2026-07-12) is a normal ADDITIVE step** — the traverser/path DSL gained the
    `back` edge (the reverse of `straight`); a v10 recipe doesn't use it and reproduces unchanged, so the
    ...
    with the owner first** — the gap is deliberate. **Pre-release exception (2026-06-29, again 2026-07-03):**
...
- `src/tutorial/` — the **guided-walkthrough layer** (the "tutorialization" of the Canvas). `script.ts` is
  ...
  **Mobile (< 64rem):** `placeBubble` returns the literal `'banner'` for any DOM-anchored bubble (after the
  `center`/`canvas-top` early returns, so those stay put) — `computeLayout` reads
  `window.matchMedia('(max-width: 64rem)')` fresh each measure tick (no extra state; rides the existing rAF
  loop). All `'banner'` bubbles for the step render inside ONE `.tut-banner` (`position: fixed`, bottom,
  full-width, `max-height: 45vh; overflow-y: auto`) instead of anchored — so nothing goes off-screen on a
  narrow viewport (the desktop anchor math sets a `right`/`left` bubble's `left` off-screen with no X clamp
  when panels are full-width). Bottom-pinned because 4 steps anchor to the top toolbar and a top banner
  would cover the Step/Play button being taught. The banner is the ONE bubble container with
  `pointer-events: auto` (so it's touch-scrollable) and carries the same `onClick` the catchers do (copy +
  advance/block-hint); the inner `.tut-bubble`s keep `pointer-events: none`, so a tap on bubble text still
  resolves to the banner's handler (mirrors bubble-over-catcher). `BubbleBody` is the shared inner markup
  (a **`.tut-bubble-num` "N/total" step badge** top-right — muted, 0.72rem, fed the controller's
  `index`/`total` so a bubble is referable as "step 3/11", on every chapter; the first paragraph reserves
  right padding so a long line can't run under it — then the finale check + paragraphs + code + continue
  affordance) used by both the desktop per-bubble render and
  the banner; `bubbleRefs` stays one-ref-per-`step.bubbles[i]`, populated by whichever of the two
  mutually-exclusive paths renders that index, so `copyBubbleAt` is unchanged. Desktop (≥ 64rem) is
  byte-identical. `.tut-message` (the block-hint toast) flips to top-anchored on mobile so it can't collide
  with the bottom banner.
  ...
  **Scripted stage setup (`SceneSetup`, added for Chapter 2 "Colorings"):** a step may carry a `setup` — a
  one-shot declarative mutation applied by Workspace's `scene` effect when that step becomes active (keyed on
  the step id, passed via `handle.scene = {key, setup}`). Fields: `openLeft`, **`editTraverser`** (a def NAME
  to open in the Traversers full-pane editor so its code is visible — Workspace stores it in `tutEditDef`,
  persisted across steps, and passes it to `TraversersPane` as `forceEditName`; an effect there opens the
  matching def once it's in the store), `defs` (traverserStore.setAll),
  `seeds` (placed walkers with an explicit `heading` edge number), `coloring` (coloringStore.setAll), `stop`,
  `prefill` (run the seeds to completion via `runToCompletion` and `setOverlay` the finished board, so the
  colorings have data to paint without the user pressing Play), and **`play`** (start a LIVE run from the
  seeds when the step activates — mirrors `initRun` but from the local built walkers: it sets
  `runLive`+`running` so the run clock animates the fill in real time; used by the ordering step so the user
  watches it play out, vs `prefill`'s instant finished board). The applier compiles the scene's `defs`
  LOCALLY for each seed's settings — the `defs` memo hasn't picked up the `setAll` yet in the same render, so
  it can't be read. **Run-finished detection:** a natural finish leaves `runLive` a non-null EMPTY array (all
  walkers died), so the signal `runEnded = runLive !== null && runLive.length === 0` (distinct from a manual
  Step-pause, where walkers survive). The other Chapter-2 signals are `coloringRuleCount` / `firstRuleColorHex`
  / `firstRuleIsRamp`. **Composite order gotcha:** the colorizer stacks rules top→bottom with the LAST rule on
  top, so a covering (opaque) rule is the BOTTOM of the list, not the "top" — Chapter 2's ordering lesson
  places the cover last and teaches this.
...
- `src/components/{PredicatePane,ColoringPane,ColorField,ColorPicker,ReorderableList,TrashButton}.tsx`
  ...
  ramp — `toRamp` defaults the fade to **`latest-step` mod 10**, changed 2026-07-12 from `visited`/6); the
  colour picker is the native input (opacity lives on the **rule**, not per colour). `ColorPicker` +
  `ColorField`'s flat `AddColour` take an optional `dataTut` so the tutorial can spotlight the flat swatch
  (`rule-swatch`) + the flat `+` (`add-color`); the Coloring pane's "+ Add rule" carries `add-coloring-rule`.
  **`ColorPicker` commits on the native `change` event, NOT React's `onChange`** (2026-07-13 lag fix): React's
  `onChange` on `<input type="color">` binds to the DOM **`input`** event, which streams continuously while
  you drag in the OS colour dialog — each frame would `store.replace` → re-`colorize` the whole tiling + redraw
  → lag. So the input is **uncontrolled** (`defaultValue`), a `change` listener (fires on picker-dismiss) is
  wired via a ref, and an effect syncs the swatch when `value` changes from elsewhere (dice / ramp edits /
  reopen). **Don't revert it to a controlled `value` + `onChange`** — that re-introduces the per-drag-frame
  recolour. Any future value picker that feeds an expensive recompute wants the same commit-on-`change` shape.
...
- **Tile numbering is the user's, never generation order** (`src/tiling/numbering.ts`; a dropdown in the
  "canvas settings" section of Inspect). Three schemes, all working on any tiling from geometry + adjacency:
  **`left-to-right`** (reading order — rows top→bottom via single-linkage y-clustering, each row left→right;
  NOT `tiling.nodes` order, which numbers hexagons column-first), **`spiral`** (a true flowing spiral, built
  as a greedy **wall-following WALK**, `buildSpiral`: from the centre tile, each step goes to the unvisited
  neighbour reached by the SHARPEST RIGHT turn from the current heading — maximise the signed clockwise turn,
  a ~180° reversal disqualified — i.e. the classic right-hand rule, which on a filled convex patch hugs the
  visited blob and winds outward as one connected +1 path, consecutive numbers edge-adjacent; a dead-end
  resumes from the nearest unvisited tile. **A spiral is a WALK, not sorted rings** — two earlier ring-based
  cuts both misordered (a geometric arm-SORT `round()`-stranded tiles a whole winding away; BFS-rings-swept-
  by-angle mis-sequenced within AND between rings) — so don't reintroduce ring sorting for `spiral`),
  **`radial`** (concentric rings by exact distance, then
  clockwise angle — right rings but the numbering jumps back to the top each ring; this is what the first cut
  wrongly called "spiral"). Each gives a tile ONE number, and that number is EVERY user-facing surface: the stats
  label + Inspect "Tile #N" (`numberOf`), the **`tile-number` DSL attribute**, **`.tile N`** addressing, and
  **`find-lowest/highest-tile`**'s ordering. Mechanism: `indexById` (threaded through EvalContext /
  ...
  So DON'T reintroduce `tiling.nodes` order for anything the user sees — raw generation order is only an
  internal tile-id detail, surfaced as NO number. The `Numbering` bundle `{order, posOf}` (`numberingFor`)
  is threaded into the run (TraverseState) for `find-lowest/highest`; recipe stores `numberingScheme`
  (schema v9) so a non-default creation reproduces.
- **The DSL path separator is `.` (a dot), NOT `@`** (changed 2026-07-11; §6/§7). A `.`-path reads/hops to
...
- **The DSL `orientation` attribute is NOT the lattice "orientation" coordinate — and a lattice label must
  never reuse an Inspect field name** (the "two orientations" fix, 2026-07-16). Two different numbers describe
  a tile's rotational variant: (1) the **DSL `orientation` attribute** (`src/tiling/orientation.ts`) — derived
  from geometry (ranked rotation bucket), portable across tilings, and **what every DSL `orientation` read
  resolves to**; (2) a per-tiling **lattice discriminator coordinate** (the 3rd `lattice` entry), an internal
  slot/class index baked into the tile id. They can number the same variants differently — rhombille's lattice
  `0/1/2` = DSL orientation `2/0/1`. rhombille + triangular used to *label* their coordinate "orientation" too
  (`latticeLabels`), so Inspect showed the word twice with disagreeing values and a directive reading
  `orientation` looked broken (it wasn't). Now labelled `face` / `facing`. **`latticeLabels` is display-only**
  — the DSL reads lattice via `coordinate[n]` by INDEX, never by label — so renaming is free (no tile-id /
  recipe / gallery impact). `lattice-uniqueness.test.ts` asserts no `latticeLabels` entry collides with a
  fixed Inspect row (`orientation` / `tile type`); **don't name a new tiling's coordinate "orientation"**
  (or "tile type"). To route by rotational variant in the DSL, use the `orientation` attribute (read its value
  off the single Inspect "orientation" row), not `coordinate[n]`.
...
- **The traverser decision log** lives in `src/components/DebugPane.tsx` (+ `.css`) with the pure
  ...
  **opt-in** `TickTrace` (`src/traverse/trace.ts`, built only by `stepTraversersTraced`). It now sits at
  ...
  A **`put`/`increase` records its resolved targets** in the trace (`StmtTrace` `write` gained
  `targets: WriteTargetTrace[]`, filled in `exec.ts`'s `applyWrite` — each element's resolved tile id, or
  null for a walker register / off-grid `.`-path); `DebugPane` renders them as hoverable sub-rows (mirrors
  a move's candidate list) and `highlights.ts` maps them — `statementGroups`' `write` case lights ALL
  written tiles (magenta **`write`** role), `writeTargetGroups` lights ONE. So an off-grid target visibly
  writes nowhere — the debugging point of the feature. The overlay **pulses**: `drawHighlights` takes a
...
**A redeploy strands an already-open tab's export (the "export worker failed" mystery), and why we DON'T
fall back.** The worker is its OWN content-hashed chunk (`exportWorker-<hash>.js`), and — being a `new
Worker(new URL('./exportWorker.ts', import.meta.url))` — it's **fetched lazily on the FIRST export**, with
that hashed filename baked into the main bundle. Cloudflare Pages serves only the latest deploy's files, so
after a redeploy the old hash 404s. A tab opened BEFORE the redeploy still runs the old main bundle, which
requests the now-gone old worker URL → the worker can't load → `worker.onerror` fires with an **empty
ErrorEvent** (this is the intermittent "export worker failed" the export debug log was built to catch — a
BARE onerror, no structured `stage`, distinct from a compute/render throw which the worker's try/catch turns
into a structured `error` message). A hard reload fetches the current main bundle + its matching worker
chunk. **Handling (`exportImage.ts` `viaWorker`):** a bare `onerror` OR a `new Worker` construction throw
rejects with `WORKER_UNAVAILABLE_MESSAGE` (flagged `workerUnavailable`; `isWorkerUnavailable(e)`), and
`Workspace`'s export `.catch` shows it as a toast + **skips the debug log** (a reload is the remedy, not a
bug report); a structured/staged failure keeps the log. We deliberately do **NOT** silently re-run on the
main thread (owner rejected masking the failure). The real ROOT fix — so exports survive a redeploy — is to
**inline the worker** (`import W from './exportWorker?worker&inline'`) so it's a Blob from in-memory source
with no separate fetch to 404; the owner declined it for now (it embeds the ~105 KB worker into the main
bundle, ~+40 KB gzip for everyone). Revisit if surviving-redeploy silently becomes worth that cost. This is
NOT a browser-capability fallback: `canUseWorker()` still routes browsers with no `Worker`/`OffscreenCanvas`
to `viaMainThread` up front — that's a different, legitimate path, not failure-masking.

**Two run limits, and only ONE of them is surfaced — keep `max-steps` ≥ `maxTicks`.** A run has two caps:
the per-walker **`max-steps`** (`DEFAULT_SETTINGS.maxSteps`, `src/traverse/lang/types.ts`) drops an individual
walker once its step count exceeds it; the run-wide **`maxTicks`** (`runToCompletion`, `1_000_000`) stops the
whole loop. **Only a `maxTicks` stop is reported** (`RunResult.hitCap` → the export "!" badge); a walker
retiring on `max-steps` just leaves `walkers.length === 0`, i.e. `hitCap === false` — indistinguishable from a
natural finish. So a `max-steps` default LOWER than `maxTicks` silently truncates any single-walker fill
pattern (one tile per step) at that many tiles — a `find-lowest-tile` walker on a big export froze in a
√50 000 ≈ 224² centred blob until the default was raised to `1_000_000` to match `maxTicks` (2026-07-09; §6/§7).
...
**Gallery spotlight is a deep-linkable hash sub-route.** Opening a creation sets `#/gallery/<id>`; the router
still resolves it to the gallery page (`parse` matches `startsWith('gallery')`), and `Gallery` reads the id off
the hash via **`useSpotlightId`** (`useSyncExternalStore` on `hashchange`) rather than local state — so a direct
link, the Share button, and Back/Forward all drive the same single source of truth. Router helpers:
`gallerySpotlightHref(id)` builds the hash (encodes the id), `spotlightIdFromHash(hash)` reads it back off the
**RAW hash** (not the lower-cased route path, so a case-sensitive id survives). A shared link can point at a
creation that isn't on the recipient's loaded feed page, so `Gallery` fetches just that one via
**`GET /api/creations/:id`** (`functions/api/creations/[id]/index.ts` → `mapRow`; client `fetchCreation`) when
the id isn't in `feed.items` — the feed copy still wins when present (its upvote count stays live). An
`index.ts` inside the existing `[id]/` directory (beside `recipe.ts`/`upvote.ts`) is what maps to the bare
