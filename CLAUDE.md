...
- **Expanded tiling set (done 2026-07-04, `8b13473`):** four more tilings beyond the 11 uniform + kalleboda —
  ...
  *Next tilings (owner asked, flagged hard):* the **aperiodic** Penrose and einstein-**hat** monotile — need
  substitution/deflation generators, not lattices (§8).
- **Next up:** **aperiodic tilings** (Penrose + hat, §8) · **`exploroboros.io` custom domain** (register +
  attach — §8) → **DSL-driven traversers** (custom rules in the Traversers pane — paint/move/visit/split/
...
- [ ] **Generic tiling render + data model** *(§4.3)*
  ...
  - [ ] **Penrose tiling** (https://en.wikipedia.org/wiki/Penrose_tiling) — *aperiodic*, so NOT a periodic
    lattice: generate by **substitution / deflation** (P3 thick+thin rhombi, or P2 kite+dart) to a depth, emit
    the polygons, weld + `stitch()`. Rhombi ARE edge-to-edge so stitch should pair them; the hard part is the
    recursive generator + keeping it deterministic. Owner request 2026-07-04 (flagged as challenging).
  - [ ] **Einstein "hat" monotile** (https://en.wikipedia.org/wiki/Einstein_problem) — *aperiodic single tile*
    (2023). The hat is a 13-gon polykite; the tiling needs **reflected copies** and a **metatile substitution**
    (H/T/P/F) to generate. Check whether it's edge-to-edge enough for `stitch()` (hats can meet mid-edge → may
    need the T-junction handling that's currently out of scope). Owner request 2026-07-04 (flagged as challenging).
