...
- `src/export/` — the **pure, isomorphic image-export core** (no React/Konva), public API via
  ...
  - **Recipe versioning (so images survive app updates).** The recipe carries `schemaVersion` (the
    ...
    image. Current exports are `schemaVersion: 5` (`MIGRATIONS` holds v1→v2 output-size + v2→v3 the empty
    ...
    image is unchanged since names map to ids). **The live gallery's stored `recipe_json` was ALSO rewritten
