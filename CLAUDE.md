...
- **DSL path separator changed from `@` to `.` (2026-07-11):** the "read/hop to another tile" separator is now
  ...
  **Lexing stays unambiguous** because the precedence already in both lexers resolves the two collisions: the
  ...
  recipes + every DSL example were rewritten. **Live data survives** via recipe **schema v9 → v10**, whose
  migration (`rewritePathsV10`) rewrites `@`→`.` in the recipe's DSL-text fields (traverser/predicate text,
  inline coloring predicates, initial-state) — so old PNGs and gallery rows migrate on read (`parseRecipe`,
  used by both PNG import and `fetchRecipe`). `tools/migrate-paths.mjs` additionally rewrites the live D1's
  stored `recipe_json` at rest (dry-run default; owner-gated `--apply --remote`, run only after the v10
  frontend is deployed). See §7 + the §9 lexer/recipe notes.
...
- `src/export/` — the **pure, isomorphic image-export core** (no React/Konva), public API via
  ...
  - **Recipe versioning (so images survive app updates).** The recipe carries `schemaVersion` (the
    ...
    **+ v9→v10 (2026-07-11), a TEXT rewrite** — the DSL path separator changed from `@` to `.`, so the
    migration (`rewritePathsV10`) replaces `@`→`.` in the recipe's DSL-text fields (`traversers[].text`,
    `predicates[].text`, inline `coloringRules[].predicate.text`, `initialState`) — `@` was only ever the
    path operator there, so the per-field replace is exact + idempotent. This is why the always-bump rule
    matters: old PNGs and gallery rows carry `@` text and MUST migrate on read or they'd fail to compile.
    Current exports are **`schemaVersion: 10`**. **The live gallery's stored `recipe_json` was ALSO rewritten
    once via `tools/migrate-names.mjs --apply`** (idempotent; a name-only transform, leaving each row's schema
    version for on-read migration) — do the same if names ever need another sweep; **`tools/migrate-paths.mjs`
    is the `@`→`.` counterpart** (same shape as migrate-names; text-only, leaves each row's version for on-read
    migration since prod rows span v3..v9; run `--apply --remote` only AFTER the v10 frontend deploys). **Pre-release exception (2026-06-29, again 2026-07-03):**
